...
\end{lstlisting}
  \vspace{0.6em}
  \aside{Вторая сборка прошла заново: другая конфигурация --- другая папка в \texttt{bin}.
    А в \texttt{out} --- никаких исходников, никакого \texttt{obj}, только результат.
    Это то, что \hi{поедет на сервер}.}
\end{frame}
